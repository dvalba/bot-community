...
def init_db():
    """Crea las tablas si no existen y migra la tabla usuarios si es vieja."""
...
def seed_db():
    """Rellena secciones y socios por defecto. Idempotente."""
    with _connect() as conn:
        for clave, titulo, contenido, orden in SECCIONES_DEFAULT:
            conn.execute("""
        ...
        for socio_id, nombre, porcentaje in SOCIOS_DEFAULT:
            conn.execute("""
...
async def configurar_comandos():
    """Registra los comandos en Telegram. Si falla la red, no rompe el bot."""
    try:
        ...
        print(f"⚠️  No se pudieron registrar los comandos en Telegram: {e}")
        print("   El bot seguirá arrancando igual. Revisa tu red/DNS.")
